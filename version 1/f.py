...
#     FIXED: Using correct model names and enhanced error handling
#     """

#     def __init__(self, api_key: str = None, model_name: str = "gemini-1.5-pro"):
#         """
#         Initialize Gemini-only product extractor
#         FIXED: Using stable model names
#         """
...
#         if not self.api_key:
#             raise ValueError("❌ GOOGLE_AI_API_KEY is required. Set it in environment variables or pass directly.")

#         self.setup_gemini_client()

#     def setup_gemini_client(self):
#         """Initialize the Gemini client with correct model names"""
#         try:
...
#         try:
#             # ===================================================================
#             # ## CHANGE 1: Using the more precise Gemini prompt
#             # ===================================================================
...
# - specifications: A dictionary of key-value technical specs (e.g., {{"Dimensions": "H 35 x W 34", "Material": "Leather"}}).
...
#     """
#     COMPLETE FIXED VERSION of Gemini-enhanced product scraper
#     All issues resolved: model names, image extraction, fallback extraction
#     """

#     def __init__(self, brand_id="your_brand", polite_delay=1, output_dir="gemini_scrape_output",
...
#             self.logger.info(f"✅ Gemini extractor initialized successfully with model: {self.gemini_extractor.model_name}")
#         except Exception as e:
...
#         self.logger.info(f"🚀 Gemini-Enhanced Scraper initialized. Output: {self.output_dir}, Brand: {self.brand_id}")
...
#         self.logger.error(f"❌ FAILURE in {step}: {url} - {error}")
...
#         except Exception as e:
#             self.log_failure('url_extraction', url, e, {'step': 'fetch_soup'})
#             return None

#     def is_product_url(self, url):
#         if not url: return False
#         path = urlparse(url).path.lower()
#         exclude_patterns = ['/category', '/tag', '/cart', '/checkout', '/search', '/account', '/login', '/register', '/contact', '/about', '/privacy', '/terms', '/blog', '/news', '/wp-admin', '.xml', '.json', '.css', '.js', '.png', '.jpg', '.gif', '.svg', '.pdf', '?add-to-cart', '#', '/policies/']
#         if any(pattern in path for pattern in exclude_patterns): return False
#         product_patterns = ['/product/', '/shop/', '/products/', '/item/']
#         return any(pattern in path for pattern in product_patterns)

#     def detect_platform(self, soup):
#         if soup.select('.woocommerce, li.product.type-product'): return 'woocommerce'
#         if soup.select('.product-card, .grid-product__link, .shopify-section'): return 'shopify'
#         body_classes = soup.select_one('body')
#         if body_classes:
#             body_class_str = ' '.join(body_classes.get('class', []))
#             if 'woocommerce' in body_class_str: return 'woocommerce'
#             if 'shopify' in body_class_str: return 'shopify'
#         return 'custom'
...
#                 for item in soup.select('li.product a[href]'):
#                     full_url = urljoin(base_url, item.get('href'))
#                     if self.is_product_url(full_url): links.add(full_url)
#             elif platform == 'shopify':
#                 for selector in ['.product-card a[href*="/products/"]', '.grid-product__link', 'a[href*="/products/"]']:
#                     for element in soup.select(selector):
#                         full_url = urljoin(base_url, element.get('href'))
#                         if self.is_product_url(full_url): links.add(full_url)
#             else:
#                 for selector in ['a[href*="product"]', 'a[href*="item"]', '.product a[href]']:
#                     for element in soup.select(selector):
#                         full_url = urljoin(base_url, element.get('href'))
#                         if self.is_product_url(full_url): links.add(full_url)
#         except Exception as e:
#             self.log_failure('url_extraction', base_url, e, {'step': 'find_product_links'})
#         return links

#     def get_next_page_url(self, soup, base_url, current_page, platform):
#         try:
#             selectors = {
#                 'woocommerce': ['a.next.page-numbers', 'a[aria-label="Next"]'],
#                 'shopify': ['a[aria-label="Next page"]', '.pagination__item--next a']
#             }.get(platform, [])
#             for selector in selectors:
#                 next_element = soup.select_one(selector)
#                 if next_element and next_element.get('href') and 'javascript' not in next_element.get('href').lower():
#                     return urljoin(base_url, next_element.get('href'))
#             if current_page < 25:
#                 if '?page=' in base_url: return re.sub(r'\?page=\d+', f'?page={current_page + 1}', base_url)
#                 else: return f"{base_url}{'&' if '?' in base_url else '?'}page={current_page + 1}"
#         except Exception as e:
#             self.log_failure('url_extraction', base_url, e, {'step': 'get_next_page_url'})
#         return None

#     def extract_product_urls(self, start_url):
#         self.logger.info(f"🚀 Starting URL extraction from: {start_url}")
#         all_products, seen_pages, current_url, page = set(), set(), start_url, 1
#         platform = 'custom'
#         while current_url and current_url not in seen_pages and page <= 50:
#             self.logger.info(f"📄 Scraping page {page}: {current_url}")
#             soup = self.fetch_soup(current_url)
#             if not soup: break
#             if page == 1: platform = self.detect_platform(soup)
#             links = self.find_product_links(soup, current_url)
#             new_links = links - all_products
#             self.logger.info(f"🔗 Found {len(links)} links, {len(new_links)} new.")
#             if not new_links and page > 1: break
#             all_products.update(links)
#             seen_pages.add(current_url)
#             next_page = self.get_next_page_url(soup, current_url, page, platform)
#             if next_page and next_page not in seen_pages:
#                 current_url, page = next_page, page + 1
...
#             writer = csv.writer(f)
#             writer.writerow(['url'])
#             for url in products: writer.writerow([url])
#         self.logger.info(f"✅ Saved {len(products)} unique product URLs")
#         return products
...
#     def fetch_page_content(self, url, output_path):
...
#                 time.sleep(3)
#                 with open(output_path, "w", encoding="utf-8") as f: f.write(page.content())
#                 browser.close()
#                 self.logger.info(f"✅ Success! Saved to '{os.path.basename(output_path)}'")
#                 return True
#         except Exception as e:
#             self.log_failure('html_fetching', url, e, {'output_path': output_path})
#             return False

#     def fetch_all_html_files(self):
#         self.logger.info(f"🔄 Starting HTML fetching from {self.urls_csv}")
#         os.makedirs(self.html_dir, exist_ok=True)
#         url_mapping = []
#         try:
#             with open(self.urls_csv, 'r', encoding='utf-8') as f:
#                 urls_to_process = [row['url'] for row in csv.DictReader(f) if row.get('url')]
#             if not urls_to_process: return False
#             self.logger.info(f"📊 Found {len(urls_to_process)} URLs to fetch.")
#             success_count = 0
#             for url in urls_to_process:
#                 filename = self.generate_filename_from_url(url)
#                 output_file_path = os.path.join(self.html_dir, filename)
#                 if self.fetch_page_content(url, output_file_path):
#                     success_count += 1
#                     url_mapping.append({'original_url': url, 'filename': filename, 'filepath': output_file_path})
#                 time.sleep(self.polite_delay)
#             with open(self.url_mapping_csv, 'w', newline='', encoding='utf-8') as f:
#                 if url_mapping:
#                     writer = csv.DictWriter(f, fieldnames=['original_url', 'filename', 'filepath'])
#                     writer.writeheader()
#                     writer.writerows(url_mapping)
#             self.logger.info(f"✅ Fetched {success_count}/{len(urls_to_process)} HTML files")
#             return success_count > 0
#         except Exception as e:
#             self.log_failure('html_fetching', 'general', e, {'step': 'fetch_all_html_files'})
#             return False

#     def process_html_files_to_csv_with_gemini(self):
#         self.logger.info(f"🤖 Starting enhanced product extraction from {self.html_dir}")
#         files_processed, successful_extractions, results = 0, 0, []
...
#         url_mapping = {}
#         if os.path.exists(self.url_mapping_csv):
#             try:
#                 with open(self.url_mapping_csv, 'r', encoding='utf-8') as f:
#                     url_mapping = {row['filename']: row['original_url'] for row in csv.DictReader(f)}
#                 self.logger.info(f"📋 Loaded {len(url_mapping)} URL mappings")
#             except Exception as e:
#                 self.logger.warning(f"Could not load URL mapping: {e}")
#         html_files = [os.path.join(root, fname) for root, _, files in os.walk(self.html_dir) for fname in files if fname.lower().endswith((".html", ".htm"))]
#         if not html_files: self.logger.error("No HTML files found"); return False
#         self.logger.info(f"📊 Processing {len(html_files)} HTML files")
#         for file_path in sorted(html_files):
#             files_processed += 1
#             filename = os.path.basename(file_path)
#             original_url = url_mapping.get(filename, "")
#             self.logger.info(f"🔍 [{files_processed}/{len(html_files)}] Processing: {filename}")
#             try:
...
#                 product_record = self.convert_to_csv_format(product_data, original_url, base_folder_path)
...
#         self.logger.info(f"🎯 EXTRACTION COMPLETE: {successful_extractions}/{files_processed} successful ({success_rate:.1f}%)")
...
#             saved_image_names = []
#             for idx, image_url in enumerate(list(image_urls)):
...
#     # ===================================================================
#     # ## CHANGE 2: Merged the new logic into this method
#     # ===================================================================
#     def convert_to_csv_format(self, product_data: ProductSchema, original_url: str, base_folder_path: str) -> Dict:
#         product_id = f"gm_{self.brand_id}_{uuid.uuid4().hex[:6]}"
#         product_name = f"{self.brand_id.title()} {product_data.title.title()} {product_id.split('_')[-1]}"

#         # --- NEW: Data cleaning and separation logic from Program 2 ---
...
#                     phrases_to_remove.append(k) # Collect keys to remove from description
...
#         clean_description = product_data.description or ""
#         if clean_description:
#             # Add common headers to the list of things to remove
#             common_headers = ["Specification", "Dimensions", "Assembly", "Key Features", "Primary Material", "Secondary Material", "Room Type"]
#             phrases_to_remove.extend(common_headers)

#             for phrase in set(phrases_to_remove):
#                 # This REGEX removes lines that start with the phrase (and optional colon)
#                 # to avoid accidentally removing keywords from the middle of a sentence.
#                 clean_description = re.sub(rf'^\s*{re.escape(phrase)}:?.*?\n', '', clean_description, flags=re.IGNORECASE | re.MULTILINE)
            
#             # Tidy up whitespace and set a default if it becomes empty
#             clean_description = '\n'.join([line.strip() for line in clean_description.split('\n') if line.strip()]) or "Not found"
#         # --- End of new logic ---
...
#         )
#         self.logger.info(f"✅ Saved {len(saved_image_names)} images from the live URL for this product.")
        
#         additional_attrs = product_data.additional_attributes or {}
        
#         return {
#             "brand_id": self.brand_id, "productrange_simple": product_data.category or "", "product_name": product_name,
#             "product_id": product_id, "category_level1": product_data.category or "", "category_level2": "",
#             "category_level3": "", "price_choice": "", "min_price": product_data.price or "", "price_unit": "",
#             "original_price": product_data.original_price or "", 
#             "description": clean_description, # Using the cleaned description
#             "product_images": "|".join(saved_image_names), "video": "", "brouchure_pdf": "", "youtube": "",
#             "other_information": other_information, # Using the built other_information
#             "color": additional_attrs.get('color', ''), "shape": "",
#             "material": additional_attrs.get('material', ''), "application": "", "finish": additional_attrs.get('finish', ''),
#             "type": additional_attrs.get('type', ''), "grade": additional_attrs.get('grade', ''),
...
#     def run_complete_pipeline_with_gemini(self, category_urls):
#         self.logger.info("🚀 Starting COMPLETE Enhanced Gemini Pipeline")
#         self.logger.info("="*80 + "\n📍 STEP 1: Extracting Product URLs\n" + "="*80)
#         all_urls = set()
#         for i, url in enumerate(category_urls, 1):
#             self.logger.info(f"📍 Processing Category {i}/{len(category_urls)}: {url}")
#             try:
#                 all_urls.update(self.extract_product_urls(url))
#                 if i < len(category_urls): time.sleep(self.polite_delay * 2)
#             except Exception as e:
#                 self.log_failure('url_extraction', url, e)
#         if all_urls:
#             with open(self.urls_csv, 'w', newline='', encoding='utf-8') as f:
#                 writer = csv.writer(f)
#                 writer.writerow(['url'])
#                 for url in sorted(all_urls): writer.writerow([url])
#             self.logger.info(f"✅ Total unique URLs collected: {len(all_urls)}")
#         else:
#             self.logger.error("❌ No URLs collected. Pipeline stopped.")
#             self.save_failure_report(); return False
#         self.logger.info("\n" + "="*80 + "\n📥 STEP 2: Fetching HTML Files\n" + "="*80)
#         if not self.fetch_all_html_files():
#             self.logger.error("❌ HTML fetching failed."); self.save_failure_report(); return False
#         self.logger.info("\n" + "="*80 + "\n🤖 STEP 3: Enhanced Product Detail Extraction\n" + "="*80)
#         if not self.process_html_files_to_csv_with_gemini():
...
#     BRAND_ID = "furniture_brand"
#     CATEGORY_URLS = ["https://bantia.in/collections/1-seater-recliners"]
...
#         brand_id=BRAND_ID,
#         polite_delay=1,
#         output_dir="complete_gemini_scrape_output",
...
#     success = scraper.run_complete_pipeline_with_gemini(CATEGORY_URLS)
...
# # ===================================================================
# # ## UPDATED SECTION: Master Lists for Validation
# # ===================================================================
# # The following lists have been updated with the comprehensive values you provided.
# MASTER_VALUES = {
#     "colors": [
...
#             self.logger.info(f"Found {len(image_urls)} high-quality product image URLs after filtering.")
#             saved_image_names = []
#             # This loop now iterates over ALL unique images, not just the first 8.
#             for idx, image_url in enumerate(list(image_urls)):
#                 file_name = f"{product_id}_{idx + 1}"
#                 saved_name = self.download_image(image_url, image_folder_path, file_name)
#                 if saved_name: saved_image_names.append(saved_name)
...
#         # --- Data cleaning and separation logic for description/other_information ---
#         other_information_parts = []
#         phrases_to_remove = []
...
#         # --- Validation logic for color, material, shape, type ---
        
#         # Consolidate all attributes into a single dictionary with lowercase keys for easy lookup
#         all_attrs = {}
#         if product_data.specifications:
#             all_attrs.update({k.lower(): v for k, v in product_data.specifications.items()})
#         if product_data.additional_attributes:
#             all_attrs.update({k.lower(): v for k, v in product_data.additional_attributes.items()})

#         # Helper function for validation
#         def validate(value_key):
#             extracted_value = all_attrs.get(value_key, '')
#             if not extracted_value:
#                 return "" # Return empty if nothing was extracted
#             if extracted_value.lower() in VALIDATION_SETS[value_key]:
#                 return extracted_value # Return original value if valid
#             return f"Test {value_key.title()}" # Return placeholder if not valid

#         validated_color = validate('color')
#         validated_material = validate('material')
#         validated_shape = validate('shape')
#         validated_type = validate('type')
...
#             "application": "", 
#             "finish": all_attrs.get('finish', ''), # Can still get other attrs
#             "type": validated_type, 
#             "grade": all_attrs.get('grade', ''),
#             "service_category": "", "Remarks": f"Enhanced extraction with {self.gemini_extractor.model_name}",
#             "bw_rating": str(product_data.rating) if product_data.rating else "", "meta_keywords": "",
#             "online_links": "", "product_url": original_url, "sku": product_data.sku or "",
#             "availability": product_data.availability or "", "review_count": str(product_data.review_count) if product_data.review_count else "",
...
#     """Complete enhanced scraper example"""
#     while True:
#         BRAND_ID = input("▶️ Please enter the Brand ID for this scrape session: ")
#         if BRAND_ID and not BRAND_ID.isspace():
#             break
#         print("❌ Brand ID cannot be empty. Please try again.")
...
#         brand_id=BRAND_ID,
#         polite_delay=1,
#         output_dir="gemini_scrape_output",
#         gemini_api_key=GEMINI_API_KEY,
#         gemini_model="gemini-1.5-pro"
#     )
...
class EnhancedProductScraperWithGemini:
    ...
    def extract_product_urls(self, category_task):
        self.logger.info(f"🚀 Starting URL extraction from: {category_task['url']} (Category ID: {category_task['category_id']})")
        all_products, seen_pages, current_url, page = set(), set(), category_task['url'], 1
        platform = 'custom'
        product_tasks = []
        while current_url and current_url not in seen_pages and page <= 50:
            self.logger.info(f"📄 Scraping page {page}: {current_url}")
        ...
        for url in sorted(all_products):
            product_tasks.append({
                'url': url,
                'category_id': category_task['category_id'],
                'brand': category_task['brand']
            })
        self.logger.info(f"✅ Extracted {len(product_tasks)} product tasks for category {category_task['category_id']}")
    ...
    def fetch_all_html_files(self, product_tasks, max_workers):
        self.logger.info(f"🔄 Starting HTML fetching from {len(product_tasks)} product tasks")
        os.makedirs(self.html_dir, exist_ok=True)
        url_mapping = []
        try:
            worker_func = partial(self.fetch_page_content_worker, self.html_dir)
            with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = [executor.submit(worker_func, task) for task in product_tasks]
    ...
    def fetch_page_content_worker(self, html_dir, task):
        url = task['url']
        filename = self.generate_filename_from_url(url)
        output_file_path = os.path.join(html_dir, filename)
        if self.fetch_page_content(url, output_file_path):
            return {
                'original_url': url,
                'filename': filename,
    ...
    def convert_to_csv_format(self, product_data: ProductSchema, original_url: str, base_folder_path: str, category_id: str, brand: str) -> Dict:
        ...
        return {
            "brand_id": brand, "productrange_simple": product_data.category or "", "product_name": product_name,
